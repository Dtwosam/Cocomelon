...
_CURATOR_NAME = "Verified V4 Mainnet Evidence Corpus Curator"
_CURATOR_PATH = ".github/workflows/evidence-corpus-curator-v4.yml"
_CAMPAIGN_NAME = "Scheduled Genuine Mainnet Evidence Campaign V4"
...
def _int_field(payload: JsonObject, field: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise RuntimeError(f"V4 intake {field} is invalid")
    return value


def _base_intake_valid(report: JsonObject) -> None:
    if report.get("schema_version") != 1:
        raise RuntimeError("V4 intake schema version is invalid")
    if report.get("protocol") != "v4-thesis-expiry-mainnet":
        raise RuntimeError("V4 intake protocol is invalid")
    _int_field(report, "source_run_id")
    conclusion = report.get("source_conclusion")
    if not isinstance(conclusion, str) or not conclusion:
        raise RuntimeError("V4 intake source conclusion is invalid")
    if not isinstance(report.get("source_verified"), bool):
        raise RuntimeError("V4 intake source verification flag is invalid")
    if not isinstance(report.get("corpus_mutated"), bool):
        raise RuntimeError("V4 intake corpus mutation flag is invalid")
    if report.get("economic_claim") != "none":
        raise RuntimeError("V4 intake economic claim is invalid")
    if report.get("live_orders") is not False:
        raise RuntimeError("V4 intake live-order state is invalid")
...
def _intake_summary(report: JsonObject) -> str:
    ...
    if conclusion == "success":
        if verified is not True or mutated is not True:
            raise RuntimeError("successful V4 intake is not verified and admitted")
        return "accepted into V4 corpus"

    if verified is not False or mutated is not False:
        raise RuntimeError("failed V4 intake cannot be verified or mutate the corpus")
    if report.get("reason") != "source_workflow_not_successful":
        raise RuntimeError("failed V4 intake reason is invalid")
    ...
    if not isinstance(status, str) or status not in _SAFE_DIAGNOSTIC_STATUSES:
        raise RuntimeError("V4 intake diagnostic status is invalid")
    if status != "eligibility_probe":
        return f"rejected — diagnostics: {status}"

    if report.get("network_access") is not False:
        raise RuntimeError("V4 intake diagnostic network state is invalid")
    reasons = report.get("economic_ineligibility_reasons")
    if not isinstance(reasons, list) or not all(isinstance(item, str) for item in reasons):
        raise RuntimeError("V4 intake diagnostic reasons are invalid")
    if any(item not in _SAFE_REASONS for item in reasons):
        raise RuntimeError("V4 intake diagnostic reason is not whitelisted")
    for field in (
        "replay_data_complete",
        "dataset_data_complete",
        "dataset_gap_refs_empty",
        ...
        if not isinstance(report.get(field), bool):
            raise RuntimeError(f"V4 intake diagnostic {field} is invalid")
    if reasons:
...
def _report_from_zip(blob: bytes) -> JsonObject:
    with zipfile.ZipFile(BytesIO(blob)) as archive:
        ...
        if len(matches) != 1:
            raise RuntimeError("V4 intake artifact must contain one intake report")
        payload = json.loads(archive.read(matches[0]).decode("utf-8"))
    if not isinstance(payload, dict):
        raise RuntimeError("V4 intake report is not an object")
    return payload
...
def _latest_intake_report(repo: str) -> JsonObject | None:
    ...
    raw = artifacts.get("artifacts")
    if not isinstance(raw, list):
        raise RuntimeError("V4 intake artifact list is invalid")
    matches = [
        item
        for item in raw
        if isinstance(item, dict)
...
def _apply_intake_summary(patch: JsonObject, summary: str) -> JsonObject:
    ...
    prefix = "- Latest V4 source intake: **"
    ...
    if existing:
        ...
        curator_prefix = "- Latest V4 curator: **"
        curator_lines = [
            index for index, line in enumerate(lines) if line.startswith(curator_prefix)
        ]
        if len(curator_lines) != 1:
            raise RuntimeError("dashboard must contain exactly one V4 curator line")
