...
def _phase9_v3_final_verdict(state: JsonObject) -> str:
    ...
    if freeze is None:
        raise RuntimeError("V3 final verdict exists without durable freeze")
    if final.get("freeze_id") != freeze.get("freeze_id"):
        raise RuntimeError("V3 final verdict freeze id mismatch")

    freeze_snapshot_id = freeze.get("snapshot_id")
    if not isinstance(freeze_snapshot_id, str) or not freeze_snapshot_id:
        raise RuntimeError("V3 final verdict freeze snapshot is invalid")
    ...
    if protocol_state == "insufficient_evidence":
        if final.get("final_type") != "terminal_insufficient":
            raise RuntimeError("V3 terminal verdict final type is invalid")
        if final.get("economic_claim") != "phase9_readiness_only":
            raise RuntimeError("V3 terminal verdict economic claim is invalid")
        if final.get("evaluation") is not None:
            raise RuntimeError("V3 terminal verdict contains an evaluation")
        terminal_obj = final.get("terminal")
        if not isinstance(terminal_obj, dict):
            raise RuntimeError("V3 terminal verdict payload is invalid")
        terminal = terminal_obj
        if terminal.get("edge_status") != "insufficient_evidence":
            raise RuntimeError("V3 terminal verdict edge status is invalid")
        if terminal.get("economic_claim") != "phase9_readiness_only":
            raise RuntimeError("V3 terminal verdict claim is invalid")
        if terminal.get("one_shot_oos") is not True:
            raise RuntimeError("V3 terminal verdict is not one-shot OOS")
        if terminal.get("network_access") is not False or terminal.get("live_orders") is not False:
            raise RuntimeError("V3 terminal verdict violates offline-only semantics")
        if terminal.get("snapshot_id") != freeze_snapshot_id:
            raise RuntimeError("V3 terminal verdict snapshot mismatch")
        return "INSUFFICIENT_EVIDENCE (readiness-only terminal)"

    if protocol_state == "evaluated":
        if final.get("final_type") != "evaluation":
            raise RuntimeError("V3 evaluated verdict final type is invalid")
        if final.get("economic_claim") != "phase9_baseline_edge_assessment":
            raise RuntimeError("V3 evaluated verdict economic claim is invalid")
        if final.get("terminal") is not None:
            raise RuntimeError("V3 evaluated verdict contains a terminal payload")
        evaluation_obj = final.get("evaluation")
        if not isinstance(evaluation_obj, dict):
            raise RuntimeError("V3 evaluated verdict payload is invalid")
        evaluation = evaluation_obj
        if evaluation.get("evaluation_name") != "v3-phase9-evaluation":
            raise RuntimeError("V3 final verdict evaluation identity is invalid")
        ...
        if not isinstance(edge_status, str) or edge_status not in _ALLOWED_EDGE_STATUSES:
            raise RuntimeError("V3 evaluated verdict edge status is invalid")
        return edge_status.upper()

    raise RuntimeError("V3 final verdict protocol state is invalid")
...
def _load_dashboard_state(repo: str) -> JsonObject:
    # Import lazily so unit tests for verdict semantics do not perform GitHub I/O.
    from build_evidence_dashboard import _phase9_v3_state

    state = _phase9_v3_state(repo)
    if not isinstance(state, dict):
        raise RuntimeError("V3 one-shot dashboard state is invalid")
    return state
...
def apply_final_verdict(patch: JsonObject, state: JsonObject) -> JsonObject:
    ...
    verdict = _phase9_v3_final_verdict(state)
