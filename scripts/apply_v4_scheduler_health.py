...
def _latest_scheduled_run(repo: str) -> JsonObject | None:
    workflow = _CAMPAIGN_PATH.rsplit("/", 1)[-1]
    payload = _gh_value(
        repo,
        f"actions/workflows/{workflow}/runs?event=schedule&per_page=100",
    )
