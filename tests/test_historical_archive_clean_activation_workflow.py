...
def test_activation_workflow_verifies_bootstrap_before_authorization() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")

    bootstrap = source.index("Restore authenticated pre-cutover bootstrap state")
    verify_bootstrap = source.index("--verify-only", bootstrap)
    authorize = source.index("Create pre-cutover activation authorization")

    assert bootstrap < verify_bootstrap < authorize
