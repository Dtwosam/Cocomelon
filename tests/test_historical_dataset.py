...
def test_build_training_rows_supports_15m_anchor_interval(tmp_path: Path) -> None:
    root = _build_source_root(tmp_path)
    ...
    assert any(row.feature.return_15m is not None for row in rows)
    assert all(row.feature.candle_15m_age_ms == 0 for row in rows)


def test_training_rows_reject_horizon_off_the_15m_grid(tmp_path: Path) -> None:
    root = _build_source_root(tmp_path)
...
def test_export_training_dataset_records_anchor_interval(tmp_path: Path) -> None:
    ...
    payload = json.loads(
        (tmp_path / "dataset-15m" / "manifest.json").read_text(encoding="utf-8")
    )
    table = parquet.read_table(tmp_path / "dataset-15m" / "training.parquet")

    assert manifest.anchor_interval == "15m"
    assert manifest.schema_version == 2
    assert manifest.converter_version == "historical-directional-training-v2-anchor-interval"
    assert payload["anchor_interval"] == "15m"
...
def test_export_training_dataset_rejects_mixed_anchor_intervals(
    ...
    with pytest.raises(
        HistoricalDatasetIntegrityError,
        match="exactly one anchor interval",
    ):
        export_training_dataset(
