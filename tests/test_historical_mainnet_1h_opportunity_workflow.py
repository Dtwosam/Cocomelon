...
WORKFLOW = Path(".github/workflows/historical-mainnet-1h-opportunity.yml")


def test_1h_opportunity_workflow_is_free_bounded_and_paper_only() -> None:
...
def test_1h_opportunity_workflow_uses_fixed_cost_and_stability_contract() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")

    assert "cocomelon-historical-context-opportunity" in source
    ...
    assert "--min-block-rows 50" in source
    assert "--min-block-mean-net-return 0" in source


def test_1h_opportunity_workflow_is_diagnostic_only_and_persists_artifact() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")

    assert "opportunity.json" in source
    assert "stable_long" in source
    assert "stable_short" in source
