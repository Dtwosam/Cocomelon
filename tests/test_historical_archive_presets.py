...
def test_jul_sep_2026_preset_locks_first_multimonth_geometry() -> None:
    preset = JUL_SEP_2026_V1

    assert preset.name == "archive-jul-sep-2026-v1"
...
def test_preset_identity_is_deterministic_and_unknown_names_fail() -> None:
    first = JUL_SEP_2026_V1.to_dict()
    second = get_archive_experiment_preset(JUL_SEP_2026_V1.name).to_dict()

    assert first == second
    assert first["preset_id"] == JUL_SEP_2026_V1.preset_id
...
def test_preset_runner_forwards_only_frozen_values(
    ...
    assert captured["markets"] == JUL_SEP_2026_V1.markets
    assert captured["intervals"] == JUL_SEP_2026_V1.intervals
    assert captured["horizons_ms"] == JUL_SEP_2026_V1.horizons_ms
    assert captured["start_ms"] == JUL_SEP_2026_V1.start_ms
    assert captured["end_ms"] == JUL_SEP_2026_V1.end_ms
    assert captured["config"] == JUL_SEP_2026_V1.comparison_config
    assert captured["max_funding_items"] == JUL_SEP_2026_V1.max_funding_items
    assert captured["overlap_candles"] == JUL_SEP_2026_V1.overlap_candles
