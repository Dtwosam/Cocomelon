...
class PortfolioCapacityStableThresholdCandidate:
    threshold: Decimal
    overall: HistoricalPortfolioCapacityEvaluation
...
@dataclass(frozen=True, slots=True)
class PortfolioCapacityStableThresholdCalibration:
    selected_threshold: Decimal | None
    candidates: tuple[PortfolioCapacityStableThresholdCandidate, ...]
...
@dataclass(frozen=True, slots=True)
class PortfolioCapacityHorizonCalibration:
    horizon_ms: int
    calibration: PortfolioCapacityStableThresholdCalibration
...
@dataclass(frozen=True, slots=True)
class PortfolioCapacityRidgeAlphaValidation:
    alpha: Decimal
    horizons: tuple[PortfolioCapacityHorizonCalibration, ...]
...
@dataclass(frozen=True, slots=True)
class PortfolioCapacityRidgeWalkForwardFold:
    fold_index: int
    ...
    shared_validation: tuple[PortfolioCapacityRidgeAlphaValidation, ...]
    market_validation: tuple[PortfolioCapacityRidgeAlphaValidation, ...]
    shared_test: HistoricalPortfolioCapacityEvaluation
    market_test: HistoricalPortfolioCapacityEvaluation
...
@dataclass(frozen=True, slots=True)
class PortfolioCapacityRidgeWalkForwardReport:
    folds: tuple[PortfolioCapacityRidgeWalkForwardFold, ...]
...
def calibrate_portfolio_capacity_stable_threshold(
    ...
    *,
...
) -> PortfolioCapacityStableThresholdCalibration:
    ...
    candidates: list[PortfolioCapacityStableThresholdCandidate] = []
    for threshold in thresholds:
        policy = DecisionPolicy(
            min_expected_net_edge=threshold,
            min_sample_count=min_sample_count,
        )
        overall = evaluate_predicted_portfolio_capacity_policy(
            predicted,
            policy=policy,
            costs=costs,
    ...
    if eligible:
        ...
        def score(
            item: PortfolioCapacityStableThresholdCandidate,
            ...
            if worst is None or mean is None:
                raise ValueError("qualifying capacity candidate must have means")
            return (
                worst,
                mean,
    ...
    return PortfolioCapacityStableThresholdCalibration(
...
def _calibrate_horizons(
    ...
    *,
    ...
    tuple[PortfolioCapacityHorizonCalibration, ...],
    HistoricalPortfolioCapacityEvaluation,
]:
    calibrations: list[PortfolioCapacityHorizonCalibration] = []
    for horizon_ms in sorted({row.horizon_ms for row in validation_rows}):
        ...
        calibrations.append(
            PortfolioCapacityHorizonCalibration(
                horizon_ms=horizon_ms,
                calibration=calibrate_portfolio_capacity_stable_threshold(
                    model,
                    horizon_rows,
                    costs=costs,
    ...
    return resolved, evaluate_predicted_portfolio_capacity_policy(
        predicted,
        policy=policy,
        costs=costs,
        max_concurrent_positions=max_concurrent_positions,
...
def _choose_alpha(
    candidates: Sequence[PortfolioCapacityRidgeAlphaValidation],
    *,
    min_validation_trades: int,
    min_validation_mean_net_return: Decimal,
...
def _threshold_items(
    validation: PortfolioCapacityRidgeAlphaValidation,
...
def run_walk_forward_portfolio_capacity_stable_ridge(
    rows: Sequence[HistoricalTrainingRow],
    *,
    costs: ExecutionCostAssumptions,
    candidate_alphas: Sequence[Decimal],
...
) -> PortfolioCapacityRidgeWalkForwardReport:
    if max_concurrent_positions <= 0:
        raise ValueError("max_concurrent_positions must be positive")
    ...
    results: list[PortfolioCapacityRidgeWalkForwardFold] = []
    for prepared_fold in resolved.folds:
        fold = prepared_fold.split
        models = dict(prepared_fold.models)
        shared_candidates: list[PortfolioCapacityRidgeAlphaValidation] = []
        market_candidates: list[PortfolioCapacityRidgeAlphaValidation] = []
        ...
        if selected_shared is None:
            ...
            shared_test = _abstained_evaluation(
                fold.test,
                max_concurrent_positions=max_concurrent_positions,
            )
            ...
            shared_test = evaluate_predicted_portfolio_capacity_policy(
                predicted,
                policy=policy,
                costs=costs,
                max_concurrent_positions=max_concurrent_positions,
            )
            shared_breakdowns = portfolio_capacity_trade_breakdowns(shared_test)

        if selected_market is None:
            market_alpha = None
            market_thresholds: tuple[tuple[int, Decimal | None], ...] = ()
            market_test = _abstained_evaluation(
                fold.test,
                max_concurrent_positions=max_concurrent_positions,
            )
            ...
            market_test = evaluate_predicted_portfolio_capacity_policy(
                predicted,
                policy=policy,
                costs=costs,
                max_concurrent_positions=max_concurrent_positions,
            )
            market_breakdowns = portfolio_capacity_trade_breakdowns(market_test)
    ...
    return PortfolioCapacityRidgeWalkForwardReport(folds=tuple(results))
