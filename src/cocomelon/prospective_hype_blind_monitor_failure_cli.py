...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cocomelon-prospective-hype-blind-monitor-failure",
        description="Build a redacted immutable blind-monitor failure receipt",
    )
    parser.add_argument("--audited-at-ms", required=True, type=int)
    parser.add_argument(
        "--stage",
        required=True,
        choices=("discovery", "download", "build"),
    )
    parser.add_argument("--reason-code", required=True)
    parser.add_argument("--health-artifact-id", type=_optional_artifact_id)
    parser.add_argument("--lineage-artifact-id", type=_optional_artifact_id)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        receipt = build_prospective_blind_monitor_failure(
            ...
            health_artifact_id=args.health_artifact_id,
            lineage_artifact_id=args.lineage_artifact_id,
