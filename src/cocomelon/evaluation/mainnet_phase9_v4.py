...
V4_SNAPSHOT_NAME = "v4-phase9-frozen-snapshot"
V4_EVALUATION_NAME = "v4-phase9-evaluation"
V4_CANDIDATE_NAME = "v4-baseline-fixed"
V4_SOURCE_PROTOCOL: dict[str, object] = {
    "schema_version": 1,
    "protocol": "v4-thesis-expiry-mainnet",
    "pinned_code_revision": "28db668048a83da7f7b1ba92ae2cf50aa980cb6e",
...
def _read_v4_corpus_protocol(corpus_root: str | Path) -> dict[str, object]:
    root = Path(corpus_root).resolve()
    protocol = _read_mapping(root / "protocol.json", "V4 corpus protocol")
    if protocol != V4_SOURCE_PROTOCOL:
        raise MainnetPhase9Error("V4 corpus protocol identity is invalid")
    return protocol


def _v4_candidate_from_sources(
    journal_path: Path,
    run_ids: tuple[str, ...],
) -> CandidateDefinition:
    base = _candidate_from_sources(journal_path, run_ids)
    return CandidateDefinition(
        name=V4_CANDIDATE_NAME,
...
def _v4_snapshot_hashes(root: Path) -> dict[str, str]:
    names = (
        "journal.sqlite3",
        "facts.sqlite3",
        ATTESTATION_NAME,
...
def prepare_phase9_v4_snapshot(
    corpus_root: str | Path,
    out_root: str | Path,
) -> dict[str, object]:
    corpus = Path(corpus_root).resolve()
    source_protocol = _read_v4_corpus_protocol(corpus)
    source_protocol_digest = _canonical_digest(source_protocol)
    out = Path(out_root).resolve()
    if out.exists():
        raise MainnetPhase9Error("Phase 9 V4 snapshot output must not already exist")
    if corpus == out or corpus in out.parents:
        raise MainnetPhase9Error(
            "Phase 9 V4 snapshot output must be separate from the corpus"
        )
    ...
    if code_revision != expected_revision:
        raise MainnetPhase9Error("V4 corpus code revision does not match protocol identity")
    ...
    if not run_ids or not set(run_ids).issubset(attested_run_ids):
        raise MainnetPhase9Error("selected V4 snapshot runs must be attested")
    ...
    if not isinstance(dataset_id, str) or not dataset_id.strip():
        raise MainnetPhase9Error("frozen V4 dataset id is invalid")
    ...
    if built.manifest.manifest_id != dataset_id:
        raise MainnetPhase9Error("rebuilt V4 dataset does not match frozen dataset")
    if not built.manifest.data_complete or built.manifest.gap_refs:
        raise MainnetPhase9Error(
            "selected V4 snapshot dataset must remain complete and gap-free"
        )
    ...
    candidate = _v4_candidate_from_sources(out / "journal.sqlite3", run_ids)
    profile_ids = tuple(
    ...
    if readiness.ready:
        ...
        if not isinstance(raw_split_id, str) or not raw_split_id.strip():
            raise MainnetPhase9Error("frozen V4 split id is invalid")
        split_id = raw_split_id

    snapshot_base: dict[str, object] = {
        "schema_version": SNAPSHOT_SCHEMA_VERSION,
        "snapshot_name": V4_SNAPSHOT_NAME,
        ...
        "file_sha256": _v4_snapshot_hashes(out),
...
def _verify_v4_source_protocol(
    root: Path,
    snapshot: Mapping[str, object],
) -> str:
    protocol = _read_mapping(root / "protocol.json", "V4 snapshot source protocol")
    if protocol != V4_SOURCE_PROTOCOL:
        raise MainnetPhase9Error("V4 snapshot source protocol identity is invalid")
    protocol_digest = _canonical_digest(protocol)
    if snapshot.get("source_protocol") != protocol["protocol"]:
        raise MainnetPhase9Error("V4 snapshot source protocol name is invalid")
    if snapshot.get("source_protocol_digest") != protocol_digest:
        raise MainnetPhase9Error("V4 snapshot source protocol digest is invalid")
    return protocol_digest


def evaluate_phase9_v4_snapshot(snapshot_root: str | Path) -> dict[str, object]:
    root = Path(snapshot_root).resolve()
    snapshot = _read_mapping(root / "snapshot.json", "Phase 9 V4 snapshot")
    snapshot_id = snapshot.get("snapshot_id")
    if not isinstance(snapshot_id, str) or not snapshot_id.strip():
        raise MainnetPhase9Error("V4 snapshot_id is invalid")
    base = {key: value for key, value in snapshot.items() if key != "snapshot_id"}
    if _canonical_digest(base) != snapshot_id:
        raise MainnetPhase9Error("V4 snapshot metadata digest is invalid")
    if snapshot.get("snapshot_name") != V4_SNAPSHOT_NAME:
        raise MainnetPhase9Error("snapshot is not V4 Phase 9 evidence")
    if snapshot.get("one_shot_oos") is not True:
        raise MainnetPhase9Error("V4 snapshot must be one-shot OOS evidence")
    if snapshot.get("ready_for_untouched_evaluation") is not True:
        raise MainnetPhase9Error("V4 snapshot is not ready for untouched evaluation")
    protocol_digest = _verify_v4_source_protocol(root, snapshot)
    ...
    if not isinstance(dataset_id, str) or not isinstance(split_id, str):
        raise MainnetPhase9Error("V4 snapshot dataset or split id is invalid")
    ...
    payload: dict[str, object] = {
        **result_payload,
        "evaluation_name": V4_EVALUATION_NAME,
        "snapshot_id": snapshot_id,
        "mainnet_attestation_id": snapshot["mainnet_attestation_id"],
        "source_protocol": V4_SOURCE_PROTOCOL["protocol"],
