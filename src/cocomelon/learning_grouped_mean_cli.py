...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cocomelon-learning-grouped-mean",
        description=(
            "Evaluate a frozen categorical grouped-mean challenger on an "
            "authenticated chronological validation holdout"
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        evaluation = evaluate_learning_grouped_mean(
            training_bundle,
            manifest,
        )
        write_learning_grouped_mean_evaluation(args.output, evaluation)
