...
PR #493 adds a non-economic cadence comparator to the continuous paper worker.

Purpose: measure whether the frozen V1 strategy would surface useful additional opportunities when evaluated every 5 minutes instead of only on the production 15-minute decision clock, without changing execution authority.
...
- is session-only in its first version and writes `cadence-shadow-summary.json` into each continuous-paper worker artifact;
- is fail-open with respect to trading: a comparator error disables the diagnostic but does not interrupt the ordinary paper trader.
